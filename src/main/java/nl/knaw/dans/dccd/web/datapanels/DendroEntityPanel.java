 ...
 * @author paulboon
 */
public class DendroEntityPanel extends EntityAttributePanel {
	private static final long serialVersionUID = -1882004661723730921L;
	private static Logger logger = Logger.getLogger(DendroEntityPanel.class);

	public DendroEntityPanel(String id, IModel model) {
		this(id, model, false);
	}

	public DendroEntityPanel(String id, IModel model, final boolean editable) {
		super(id, model, editable);
	}

	public class DendroEntityPanelView extends Panel {
		private static final long serialVersionUID = -571922177724503076L;

		public DendroEntityPanelView(String id, IModel model) {
			super(id, model);
	 ...
	 *
	 */
	public class DendroEntityPanelEdit extends Panel {
		public DendroEntityPanelEdit(String id, IModel model) {
			super(id, model);
			...
			// set the panel title
			add(new Label("infoTitle", mapper.getEntityLabelString(infoObject
					.getClass())));
			// populate the table
			add(getListView(entries, infoObject));
		}

		private static final long serialVersionUID = -186050537052945041L;
	}
	...
	private ListView getListView(List<UIMapEntry> entries,
		...
		return new ListView("dendro_info", entries) {
			@Override
			protected void populateItem(ListItem item) {
				...
				Label requiredIndicator = new Label("required_indicator","*");
				item.add(requiredIndicator);
				requiredIndicator.setVisible(false); // hide by default
				...
				if (panel != null ) 
				{
					...
					if (isEditable() && entry.isRequired()) 
					{
						// show indicator
						requiredIndicator.setVisible(true); 
					}
