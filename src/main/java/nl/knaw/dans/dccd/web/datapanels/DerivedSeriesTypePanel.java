 ...
 * @author paulboon
 */
public class DerivedSeriesTypePanel extends EntityAttributePanel 
{
	private static final long	serialVersionUID	= 2742852585526202775L;
	private static Logger logger = Logger.getLogger(DerivedSeriesTypePanel.class);
	
	public DerivedSeriesTypePanel(String id, IModel model) {
		super(id, model, false);
	}

	public DerivedSeriesTypePanel(String id, IModel model, final boolean editable) {
		super(id, model, editable);
		//super(id, model, false); // only allow view
	}
		
	public class DerivedSeriesTypePanelView extends Panel {
		private static final long	serialVersionUID	= 5809528172520692253L;

		public DerivedSeriesTypePanelView(String id, IModel model) {
			super(id, model);
	...
	public class DerivedSeriesTypePanelEdit extends Panel {
		private static final long	serialVersionUID	= 5017144981949600550L;
		// For the ComboBox			
		private DropDownDataSource listDS;
		
		public DerivedSeriesTypePanelEdit(String id, IModel model) {
			super(id, model);
			...
			listDS = new DropDownDataSource<String>() {
				...
				public String getName() {
			        return "derivedseries.type.list";
			    }
			    public List<String> getValues() {
			    	//return DccdVocabularyService.getService().getTerms("sample.type");
			    	// Use the project language
			    	String langCode = ((DccdSession) getSession()).getContentLanguageCode();
					return DccdVocabularyService.getService().getTerms("derivedseries.type", langCode);
			    }
